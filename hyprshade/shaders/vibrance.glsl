...
const vec3 vibranceCoefficient = rgbBalance * -vibranceStrength;

void main()
{
    vec4 pixelColor = texture(tex, v_texcoord);
    vec3 color = pixelColor.rgb;
    vec3 lumaCoefficient = vec3(0.212656, 0.715158, 0.072186);
    float luma = dot(lumaCoefficient, color);
    float maxColor = max(color.r, max(color.g, color.b));
    float minColor = min(color.r, min(color.g, color.b));
    float saturation = maxColor - minColor;
    vec3 vibranceTransform = (sign(vibranceCoefficient) * saturation - 1.0) * vibranceCoefficient + 1.0;
